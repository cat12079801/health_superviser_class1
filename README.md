...
- 出題モードとして、全問出題とカテゴリ別出題（法令（有害）／法令（一般））を用意する。
...
新しいカテゴリ（労働衛生・労働生理）を追加するときは、`data/hygiene_hazardous.json` などのファイルを作成し、`data/categories.json` で当該カテゴリの `implemented` を `true` にするだけでよい。ホームの出題ボタン・統計・正答率は `categories.json` から自動的に導出される。`id` は全ファイルを通して一意とする。詳細は [docs/categories.md](docs/categories.md) に定める。
